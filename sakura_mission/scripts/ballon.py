...
class Ballon():
	...
	def __init__(self):
		...
		self.odom_sub = rospy.Subscriber('/sakura/safe_dir', Float64, self.sdListener, queue_size = 1)
	...
	def sdListener(self, msg):
		self.safeDirection = msg.data*0.9
	...
	def lidarListener(self,msg):
		...
		self.desireAng = self.middleOfGround(msg.ranges)/180.0*3.14159
	...
	def obsMain_dc(self):
		...
		searchAng = self.desireAng

		obs_msg = obstacleService(searchAng, 2.0)
		...
		cmd.linear.x *= smoothStep(abs(error), 1.0,0.0,0.1,1.0)
